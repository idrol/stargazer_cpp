...
namespace ImGui
{
    ...
    IMGUI_API bool			RenderEngine(ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0 = ImVec2(0,1), const ImVec2& uv1 = ImVec2(1,0), const ImVec4& tint_col = ImVec4(1,1,1,1), const ImVec4& border_col = ImVec4(0,0,0,0));
...
typedef void (*ImDrawCallback)(const DrawList* parent_list, const ImDrawCmd* cmd);
...
struct DrawList {
	ImVector<ImDrawCmd> drawCmd;
	ImVector<ImDrawIdx> drawIndex;
	ImVector<ImDrawVert> drawVert;

	IMGUI_API void scaleClipRects(const ImVec2& scale) {
		for (int cmd_i = 0; cmd_i < this->drawCmd.Size; cmd_i++) {
			ImDrawCmd* cmd = &this->drawCmd[cmd_i];
